...
\begin{equation}
    A x = b
\end{equation}
onde $A \in \mathbb{R}^{n \times n}$ representa uma matriz simétrica e positiva definida (SPD), $b \in \mathbb{R}^n$ é o vetor de termos independentes, e $x \in \mathbb{R}^n$ é o vetor de incógnitas a ser computado.
...
Como descrito na literatura teórica, para valores de $\tau$ muito próximos de zero, o sistema gera uma matriz bem-condicionada e estritamente positiva definida. À medida que o valor de $\tau$ aumenta, tanto a densidade de elementos não nulos quanto o número de condicionamento pioram, fazendo com que a matriz perca a propriedade de ser positiva definida para limiares mais altos (como $\tau = 0.2$).
